...
export default async function GarvexPage() {
  const user = await requirePlatformAdmin(); const providers = await getGarvexProviderStatus(); const membership = await primaryOrg(user.id);
  const rows = membership ? await db.select({ id: projects.id, name: projects.name, repoFullName: projects.repoFullName }).from(projects).where(eq(projects.orgId, membership.org.id)).orderBy(desc(projects.updatedAt)).limit(30) : [];
