...
const NAV = [
  ...
  { href: "/admin/garvex", label: "Garvex AI" },
  ...
  return (
    <div className="flex min-h-screen">
      <aside className="hidden w-[216px] shrink-0 flex-col border-r px-3 py-4 md:flex" style={{ background: "var(--color-bg-subtle)" }}>
        <Link href="/dashboard" className="mb-5 flex items-center gap-2 px-1"><span aria-hidden style={{ width: 13, height: 13, background: "#fafafa", borderRadius: 3, display: "block" }} /><span className="text-[12.5px] font-semibold">Deployment Platform</span></Link>
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <header className="flex items-center gap-4 border-b px-5 py-2.5 md:hidden"><Link href="/dashboard" className="text-[12.5px] font-semibold">Deployment Platform</Link><Link href="/projects" className="nav-link">Projects</Link><Link href="/admin/garvex" className="nav-link">Garvex</Link><Link href="/admin" className="nav-link">Admin</Link><Link href="/admin/settings" className="nav-link">Settings</Link></header>
