  ...
  return (
    <div className="flex flex-col gap-4">
      <Panel title="Add environment variable">
        <EnvSmartForm csrf={user.csrfToken} projectId={projectId} />
        <p className="hint px-3.5 pb-3.5">
          Search a common variable name and click it, or type a custom key. Values are encrypted at rest,
          versioned on change, injected only into the selected scope, and never returned in plaintext.
        </p>
      </Panel>
      <Panel title={`Variables (${rows.length})`}>
        {rows.length === 0 ? (
          ...
          <table className="data">
            <thead>
              <tr><th>Key</th><th>Scope</th><th>Value</th><th>Version</th><th>Updated</th><th /></tr>
            </thead>
            <tbody>
              {rows.map((row) => (
                <tr key={row.id}>
                  ...
                  <td className="mono" style={{ color: "var(--color-fg-muted)" }}>••••{row.lastFour}</td>
                  <td className="mono">v{row.version}</td>
                  <td style={{ color: "var(--color-fg-muted)" }}>{timeAgo(row.updatedAt)}</td>
                  <td className="text-right">
                    <ActionButton csrf={user.csrfToken} method="DELETE" path={`/api/v1/projects/${projectId}/env?id=${row.id}`} variant="btn-danger" confirm={`Delete ${row.key}?`}>Delete</ActionButton>
                  </td>
                </tr>
              ))}
            </tbody>
