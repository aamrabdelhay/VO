...
async function resolveBaseSha(project: typeof projects.$inferSelect, token: string | null) {
  ...
  const commit = await getBranchCommit(project.repoFullName, project.productionBranch, token);
  return commit.sha;
}
...
async function complete(orgId: string, conversationId: string, messages: ChatMessage[]) {
  ...
  if (spent >= config.dailyBudgetCents && config.dailyBudgetCents > 0) throw new AIStopped("Daily AI budget exhausted");
...
export async function POST(request: Request, ctx: { params: Promise<{ projectId: string }> }) {
  ...
  return handle(async () => {
    ...
    const [conversation] = await db.insert(aiConversations).values({
      projectId,
      deploymentId: project.currentHealthyDeploymentId,
      permission: project.aiPermission,
      title: body.mode === "prepare" ? "AI editor request" : "AI editor apply",
      createdBy: user.id,
    }).returning();
    ...
    if (body.mode === "prepare") {
      ...
      try {
        ...
        const first = await complete(project.orgId, conversation.id, [
          {
            role: "system",
            content: "You are the planning layer of a coding agent. Identify only the files that must be read to fulfill the user's request. Return strict JSON: {reply:string, filesToRead:string[]}. Never invent paths. Prefer the smallest set of relevant files.",
          },
          {
            role: "user",
    ...
    const safeFiles = body.files.slice(0, 15).filter((file) =>
      typeof file.path === "string" && typeof file.content === "string" &&
      file.path.length <= 300 && file.content.length <= 150_000 &&
      !file.path.startsWith("/") && !file.path.includes("..") && !/(^|\/)(node_modules|\.git|\.env)(\/|$)/.test(file.path),
    );
    if (!safeFiles.length) throw new HttpError(400, "No safe files to apply");

    const branch = `ai-edit/${baseSha.slice(0, 7)}-${randomToken(5)}`;
    if (!(await createBranchRef(project.repoFullName, token, branch, baseSha))) {
      throw new HttpError(400, "Could not create the AI edit branch");
    }
    const commitSha = await commitFiles(project.repoFullName, token, {
      branch,
      baseSha,
      message: `feat: ${body.summary?.slice(0, 110) || "AI editor change"}`,
      files: safeFiles,
    });
    const pr = await createPullRequest(project.repoFullName, token, {
      title: `AI editor: ${body.summary?.slice(0, 90) || "Requested change"}`,
      head: branch,
      base: project.productionBranch,
      body: `Created from the VO in-site AI editor.\n\nBase commit: ${baseSha}\nCommit: ${commitSha}\n\nThe change was generated from the user's request and published to an isolated branch.`,
    });
