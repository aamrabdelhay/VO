...
type Body = { mode?: "chat" | "architect" | "research"; prompt?: string; context?: string };
...
export async function POST(request: Request) {
  return handle(async () => {
    ...
    const system = mode === "architect"
      ? "You are Garvex, the in-site autonomous engineering architect. Understand the task, reason carefully, propose an executable plan, and identify concrete files/tools that should be used. Never pretend a tool was run when it was not."
      : mode === "research"
        ...
        : "You are Garvex, a powerful multilingual AI control center inside VO. Be concise but technically precise. When the user asks for an action on VO, explain the concrete next action and avoid claiming completion until the system actually completes it.";
