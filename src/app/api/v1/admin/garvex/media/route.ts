...
async function secret(name: string) {
  ...
  if (!value) throw new Error(`${name} is not configured in VO → Settings.`);
  return value;
}
...
export async function GET() {
  return handle(async () => {
    ...
    const key = await getPlatformSecret("OPENROUTER_API_KEY");
    if (!key) return ok({ image: false, video: false, audio: false });
...
export async function POST(request: Request) {
  return handle(async () => {
    ...
    const key = await secret("OPENROUTER_API_KEY");

    if (kind === "image" || kind === "image-edit") {
      const input: Record<string, unknown> = {
        model: body.model || DEFAULT_IMAGE_MODEL,
        prompt: prompt || "Edit the supplied image while preserving the subject and overall composition.",
      };
      if (kind === "image-edit") {
      ...
      const response = await fetch(`${OPENROUTER}/images`, {
        method: "POST",
    ...
    const response = await fetch(`${OPENROUTER}/videos`, {
      method: "POST",
      headers: { Authorization: `Bearer ${key}`, "Content-Type": "application/json" },
      body: JSON.stringify({
