...
class AnthropicProvider implements AIProvider {
  ...
  async complete(messages: ChatMessage[], opts: ProviderOptions) {
    ...
    const res = await fetch(`${opts.baseUrl ?? "https://api.anthropic.com"}/v1/messages`, { method: "POST", headers: { "content-type": "application/json", "x-api-key": opts.apiKey, "anthropic-version": "2023-06-01" }, body: JSON.stringify({ model: opts.model, max_tokens: opts.maxTokens, temperature: opts.temperature, system, messages: rest.map((m) => ({ role: m.role, content: m.content })) }) });
...
class OpenAICompatibleProvider implements AIProvider {
  ...
  async complete(messages: ChatMessage[], opts: ProviderOptions) {
    const res = await fetch(`${(opts.baseUrl ?? this.defaultBase).replace(/\/$/, "")}/chat/completions`, { method: "POST", headers: { "content-type": "application/json", authorization: `Bearer ${opts.apiKey}` }, body: JSON.stringify({ model: opts.model, temperature: opts.temperature, max_tokens: opts.maxTokens, messages }) });
...
class CohereProvider implements AIProvider {
  ...
  async complete(messages: ChatMessage[], opts: ProviderOptions) {
    const res = await fetch(`${opts.baseUrl ?? "https://api.cohere.com/v2"}/chat`, { method: "POST", headers: { "content-type": "application/json", authorization: `Bearer ${opts.apiKey}` }, body: JSON.stringify({ model: opts.model, messages, temperature: opts.temperature, max_tokens: opts.maxTokens }) });
...
export async function spentTodayCents(orgId: string): Promise<number> {
  const rows = await db.execute<{ total: number }>(sql`select coalesce(sum(m.cost_cents), 0) as total from ${aiMessages} m join ai_conversations c on c.id = m.conversation_id join projects p on p.id = c.project_id where p.org_id = ${orgId} and m.created_at > now() - interval '1 day';`);
...
async function configuredCouncil() {
  const ready = await Promise.all(COUNCIL.map(async (candidate) => {
    const apiKey = await getPlatformSecret(candidate.key);
    return apiKey ? { provider: candidate.provider, model: candidate.model, apiKey } : null;
  }));
  return ready.filter((item): item is { provider: string; model: string; apiKey: string } => Boolean(item));
}
...
export async function multiAgentComplete(messages: ChatMessage[], opts?: { maxWorkers?: number; timeoutMs?: number }) {
  ...
  const settled = await Promise.allSettled(workers.map(async (worker) => {
    ...
    const request = provider.complete(messages, { model: worker.model, apiKey: worker.apiKey, temperature: 0.1, maxTokens: 4096 });
    const timeout = new Promise<never>((_, reject) => setTimeout(() => reject(new Error(`${worker.provider} timed out`)), timeoutMs));
