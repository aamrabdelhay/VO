...
export async function harvestProblems(source: "any" | "real" | "synthetic" = "any"): Promise<Problem[]> {
  ...
  if (source !== "real" && problems.length === 0) {
    const candidates = await db
      .select({ id: projects.id, orgId: projects.orgId, name: projects.name, repoFullName: projects.repoFullName, lastSuccessfulCommitSha: projects.lastSuccessfulCommitSha, buildCommand: projects.buildCommand })
      .from(projects)
    ...
    if (project?.lastSuccessfulCommitSha) {
      problems.push({
        ...
        description: `Synthetic deterministic TypeScript failure on known-passing commit ${project.lastSuccessfulCommitSha.slice(0, 7)}`,
        error: "Deterministic mutation: const __selfPracticeMutationNever: never = 1",
...
async function materializeCommit(problem: Problem, project: typeof projects.$inferSelect, workspace: string) {
  ...
  await runCommand("git", ["init", "-q"], { cwd: workspace, timeoutMs: 20_000 });
  await runCommand("git", ["config", "user.email", "ai@platform.local"], { cwd: workspace, timeoutMs: 10_000 });
  await runCommand("git", ["config", "user.name", "platform-ai"], { cwd: workspace, timeoutMs: 10_000 });
  await runCommand("git", ["add", "-A"], { cwd: workspace, timeoutMs: 60_000 });
  const baseline = await runCommand("git", ["commit", "-qm", "baseline"], { cwd: workspace, timeoutMs: 60_000 });
  if (baseline.code !== 0) throw new Error(`Sandbox baseline git commit failed: ${baseline.stderr}`);
}

async function generationConfig(problem: Problem) {
  if (process.env.OLLAMA_BASE_URL) {
    return { provider: "ollama", model: process.env.OLLAMA_MODEL ?? "qwen2.5-coder", apiKey: process.env.OLLAMA_API_KEY ?? "", baseUrl: process.env.OLLAMA_BASE_URL, dailyBudgetCents: Number.POSITIVE_INFINITY };
  }
...
async function generateRepair(problem: Problem, conversationId: string, context: string) {
  const config = await generationConfig(problem);
  const messages: ChatMessage[] = [
    {
      role: "system",
      content: "Repair code in an isolated workspace. Return strict JSON only: {summary:string,files:[{path:string,content:string}]}. Content must be the full content of each changed file. Never modify node_modules, .git, lockfiles, CI credentials, or unrelated files. The only correctness judge is execution exit codes; do not judge your own answer.",
    },
  ...
  if (config.provider !== "ollama" && (await spentTodayCents(problem.orgId)) > config.dailyBudgetCents) {
    throw new Error("Cloud self-practice generation moved the org over its existing daily AI budget");
  }
...
export async function runSelfPracticeJob(payload: Record<string, unknown> = {}) {
  ...
  try {
    ...
    const gateway = new ToolGateway({ projectId: project.id, orgId: project.orgId, conversationId: action.id, permission: "DEVELOPER" as AIPermission, workspace, deploymentId: problem.deploymentId, secrets });
    ...
    if (problem.synthetic) {
      ...
      const exitCode = (build.result as { exitCode?: number } | undefined)?.exitCode;
      syntheticFailure = String((build.result as { output?: string } | undefined)?.output ?? "");
    ...
    const context = [
      ...
      `Reported failure: ${redact(problem.error, secrets)}`,
      `Build command: ${project.buildCommand ?? "npm run build"}`,
      `Synthetic failure output: ${redact(syntheticFailure.slice(-8000), secrets)}`,
      `Workspace tree: ${JSON.stringify(tree.result).slice(0, 6000)}`,
      "Return the smallest safe repair as strict JSON.",
    ].join("\n");

    const generated = await generateRepair(problem, conversation.id, context);
    const patch = parsePatch(generated.result.text);
    ...
    const [row] = await db.execute(sql`insert into self_practice_examples (id, project_id, source, problem_description, diff, validation_output, verified, created_at) values (${crypto.randomUUID()}, ${project.id}, ${problem.source}, ${redact(problem.description, secrets)}, ${diffText}, ${validationOutput}, true, now()) returning id, project_id as "projectId", source, problem_description as "problemDescription", diff, validation_output as "validationOutput", verified, created_at as "createdAt"`);
    await db.update(aiActions).set({ status: "succeeded", provider: generated.result.provider, model: generated.result.model, costCents: generated.result.costCents, finishedAt: new Date(), summary: patch.summary ?? problem.description }).where(eq(aiActions.id, action.id));
    return { ok: true, verified: true, example: row?.rows?.[0] ?? null };
  } catch (error) {
