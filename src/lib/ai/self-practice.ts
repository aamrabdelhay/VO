...
export async function harvestProblems(source: "any" | "real" | "synthetic" = "any"): Promise<Problem[]> {
  ...
  if (source !== "synthetic") {
    ...
    for (const row of failed) {
      if (!row.deployment.commitSha || seen.has(`deployment:${row.deployment.id}`)) continue;
      seen.add(`deployment:${row.deployment.id}`);
      problems.push({
        source: "deployment",
        ...
        description: `Failed deployment ${row.deployment.id} for ${row.project.name}`,
        error: row.deployment.errorReason ?? "Deployment failed without recorded error",
        buildCommand: row.project.buildCommand ?? "npm run build",
      });
    }
    ...
    const attempts = await db
      ...
      .from(aiFixAttempts)
      .innerJoin(deployments, eq(deployments.id, aiFixAttempts.deploymentId))
      .innerJoin(projects, eq(projects.id, aiFixAttempts.projectId))
      .where(eq(aiFixAttempts.outcome, "failed"))
      .orderBy(desc(aiFixAttempts.createdAt))
      .limit(20);

    for (const row of attempts) {
      if (!row.deployment.commitSha || seen.has(`fix:${row.attempt.id}`)) continue;
      seen.add(`fix:${row.attempt.id}`);
      problems.push({
        source: "ai-fix-attempt",
        projectId: row.project.id,
        orgId: row.project.orgId,
        deploymentId: row.deployment.id,
        commitSha: row.deployment.commitSha,
        description: `Recorded failed AI repair attempt ${row.attempt.id} for ${row.project.name}`,
        error: row.deployment.errorReason ?? row.attempt.stopReason ?? "Recorded AI repair attempt failed",
        buildCommand: row.project.buildCommand ?? "npm run build",
      });
    }
  }

  if (source !== "real" && (source === "synthetic" || problems.length === 0)) {
    const candidates = await db
      .select({
        id: projects.id,
        orgId: projects.orgId,
        name: projects.name,
        lastSuccessfulCommitSha: projects.lastSuccessfulCommitSha,
        buildCommand: projects.buildCommand,
      })
      .from(projects)
    ...
    if (project?.lastSuccessfulCommitSha) {
      ...
      problems.push({
        ...
        orgId: project.orgId,
        deploymentId: null,
        commitSha: project.lastSuccessfulCommitSha,
        description: `Synthetic deterministic ${mutation.name} on known-passing commit ${project.lastSuccessfulCommitSha.slice(0, 7)}`,
        error: "Synthetic mutation will be applied only after the unmodified commit passes validation.",
...
function parsePatch(text: string): PatchResponse | null {
  ...
  try {
    const parsed = JSON.parse(candidate.slice(start, end + 1)) as PatchResponse;
...
function safeBuildCommand(command: string | null | undefined) {
  return command && BUILD_COMMANDS.has(command.trim()) ? command.trim() : "npm run build";
}

function safeTestCommand(command: string | null | undefined) {
  return command && TEST_COMMANDS.has(command.trim()) ? command.trim() : null;
}
...
async function materializeCommit(problem: Problem, project: typeof projects.$inferSelect, workspace: string) {
  ...
  for (const args of [
    ["init", "-q"],
    ["config", "user.email", "ai@platform.local"],
    ["config", "user.name", "platform-ai"],
    ["add", "-A"],
  ]) {
    const result = await runCommand("git", args, { cwd: workspace, timeoutMs: 60_000 });
    if (result.code !== 0) throw new Error(`Sandbox git setup failed: ${redact(result.stderr)}`);
  }
  const baseline = await runCommand("git", ["commit", "-qm", "baseline"], { cwd: workspace, timeoutMs: 60_000 });
  if (baseline.code !== 0) throw new Error(`Sandbox baseline commit failed: ${redact(baseline.stderr)}`);
}
...
async function generationConfig(problem: Problem) {
  if (process.env.OLLAMA_BASE_URL) {
    return {
      ...
      dailyBudgetCents: Number.POSITIVE_INFINITY,
    };
  }
...
async function generateRepair(problem: Problem, conversationId: string, context: string, secrets: string[]) {
  ...
  await db.insert(aiMessages).values({
    conversationId,
    role: "assistant",
    content: redact(result.text.slice(0, MAX_OUTPUT), secrets),
    tokensIn: result.tokensIn,
    tokensOut: result.tokensOut,
    costCents: result.costCents,
  });
  if (config.provider !== "ollama" && (await spentTodayCents(problem.orgId)) > config.dailyBudgetCents) {
    throw new Error("Cloud self-practice generation exceeded the existing per-org daily AI budget");
  }
  return result;
}
...
export async function runSelfPracticeJob(payload: Record<string, unknown> = {}) {
  ...
  const [project] = await db.select().from(projects).where(eq(projects.id, problem.projectId)).limit(1);
  ...
  const [conversation] = await db
    ...
    .values({
      projectId: project.id,
      deploymentId: problem.deploymentId,
      permission: "DEVELOPER",
      title: "Self-practice repair",
      createdBy: "system",
    })
    .returning();
  ...
  try {
    ...
    const buildCommand = safeBuildCommand(project.buildCommand ?? problem.buildCommand);
    const testCommand = safeTestCommand(project.testCommand);
    ...
    if (problem.synthetic) {
      ...
      const baselineTests = testCommand ? await gateway.call("run_tests", { command: testCommand }) : null;
      const baselineBuild = await gateway.call("run_build", { command: buildCommand });
      ...
      if (baselineInstallCode !== 0 || baselineTestCode !== 0 || baselineBuildCode !== 0) {
        throw new Error(`Synthetic candidate was not actually passing before mutation: install=${baselineInstallCode} test=${baselineTestCode} build=${baselineBuildCode}`);
      }
      const mutation = MUTATIONS.find((item) => item.name === problem.mutation) ?? MUTATIONS[0];
      await mutation.apply(workspace);
      const mutatedBuild = await gateway.call("run_build", { command: buildCommand });
    ...
    const context = [
      ...
      `Test command: ${testCommand ?? "not configured"}`,
      `Synthetic failure output: ${syntheticFailure.slice(-8000)}`,
      `Workspace tree: ${JSON.stringify(tree.result).slice(0, 6000)}`,
      "Return the smallest safe repair as strict JSON.",
    ].join("\n");

    const generated = await generateRepair(problem, conversation.id, context, secrets);
    const patch = parsePatch(generated.text);
    if (!patch) throw new Error("Attempt generation did not return a valid JSON patch");
    ...
    const tests = testCommand ? await gateway.call("run_tests", { command: testCommand }) : null;
    const build = await gateway.call("run_build", { command: buildCommand });
    ...
    const validationOutput = redact(
      JSON.stringify({ install: install.result, tests: tests?.result, build: build.result }).slice(0, MAX_OUTPUT),
      secrets,
    );

    const verified = installCode === 0 && testCode === 0 && buildCode === 0;
    if (!verified) {
      await db
        .update(aiActions)
        .set({ status: "failed", finishedAt: new Date(), summary: `Execution verifier failed: install=${installCode} test=${testCode} build=${buildCode}` })
    ...
    const inserted = await db.execute(sql`
      insert into self_practice_examples
        (id, project_id, source, problem_description, diff, validation_output, verified, created_at)
      values
        (${crypto.randomUUID()}, ${project.id}, ${problem.source}, ${redact(problem.description, secrets)}, ${diffText}, ${validationOutput}, true, now())
      returning
        id,
        ...
        diff,
        validation_output as "validationOutput",
        verified,
    ...
    await db
      ...
      .set({
        ...
        finishedAt: new Date(),
        summary: redact(patch.summary ?? problem.description, secrets),
      })
      .where(eq(aiActions.id, action.id));
    ...
    await db
      .update(aiActions)
      .set({ status: "failed", finishedAt: new Date(), summary: message.slice(0, 500) })
      .where(eq(aiActions.id, action.id));
