...
async function runDeploymentJob(deploymentId: string) {
  ...
  const [deployment] = await db.select().from(deployments).where(eq(deployments.id, deploymentId)).limit(1); if (!deployment) throw new Error("Deployment not found");
  const [project] = await db.select().from(projects).where(eq(projects.id, deployment.projectId)).limit(1); if (!project) throw new Error("Project not found"); if (deployment.status !== "QUEUED") return { ok: false, reason: `deployment already ${deployment.status}` };
  try { await transition(deployment.id, "BUILDING", { buildStartedAt: new Date() }, { event: "VERCEL_BUILD_STARTED", message: "Deployment handed to Vercel hosting" }); return await startVercelDeployment(project, { ...deployment, status: "BUILDING" }); }
...
export const handlers: Record<string, Handler> = {
  deploy: async (payload) => payload.action === "drain" ? (await drainDeployment(String(payload.deploymentId)), { drained: payload.deploymentId }) : runDeploymentJob(String(payload.deploymentId)),
  build: async (payload) => runDeploymentJob(String(payload.deploymentId)),
  "post-promotion-monitor": async (payload) => monitorAfterPromotion(String(payload.deploymentId), Number(payload.until)),
  "preview-expire": async (payload) => expirePreview(String(payload.deploymentId), "retention policy"),
...
export async function runOneJob(): Promise<boolean> {
  const job = await claimNextJob(WORKER_ID); if (!job) return false; const started = Date.now(); const handler = handlers[job.type]; const ctx = { jobId: job.id, projectId: job.projectId ?? undefined, service: "worker" };
  if (!handler) { await failJob(job, new Error(`No handler for job type ${job.type}`), 0, WORKER_ID, { retryable: false }); return true; }
  try { log.info(`Job started: ${job.type}`, ctx); const result = await handler((job.payload ?? {}) as Record<string, unknown>, job); await completeJob(job, Date.now() - started, WORKER_ID); log.info(`Job finished: ${job.type}`, { ...ctx, result: JSON.stringify(result).slice(0, 300) }); }
  catch (error) { log.error(`Job failed: ${job.type}`, { ...ctx, error: String(error) }); await failJob(job, error, Date.now() - started, WORKER_ID); }
  return true;
}

export async function drainQueue(maxJobs = 5) { let processed = 0; while (processed < maxJobs) { const worked = await runOneJob(); if (!worked) break; processed += 1; } return processed; }
