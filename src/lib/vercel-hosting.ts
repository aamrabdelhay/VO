...
import { db } from "@/db";
import { deployments, envVars, projects } from "@/db/schema";
...
async function token() {
  return process.env.VERCEL_DEPLOY_TOKEN || process.env.VERCEL_TOKEN || (await getPlatformSecret("VERCEL_DEPLOY_TOKEN")) || "";
}

async function teamQuery() {
  const teamId = process.env.VERCEL_DEPLOY_TEAM_ID || process.env.VERCEL_TEAM_ID || (await getPlatformSecret("VERCEL_DEPLOY_TEAM_ID"));
...
async function assignFreeAlias(deploymentId: string, project: Project) {
  ...
  try {
    await vercelRequest(`/v2/deployments/${encodeURIComponent(deploymentId)}/aliases${await teamQuery()}`, { method: "POST", body: JSON.stringify({ alias: preferred }) });
    return preferred;
  } catch (error) {
    const fallback = `${base}-${project.id.replace(/[^a-z0-9]/gi, "").slice(0, 8).toLowerCase()}.vercel.app`;
    try {
      await vercelRequest(`/v2/deployments/${encodeURIComponent(deploymentId)}/aliases${await teamQuery()}`, { method: "POST", body: JSON.stringify({ alias: fallback }) });
      return fallback;
    } catch (fallbackError) {
      throw new Error(`Could not assign free deployment domain: ${error instanceof Error ? error.message : String(error)}; fallback: ${fallbackError instanceof Error ? fallbackError.message : String(fallbackError)}`);
    }
  }
}
...
export async function syncVercelDeployment(deployment: Deployment) {
  ...
  if (current.readyState === "READY" && deployment.status !== "PROMOTED") {
    ...
    if (project) { try { freeUrl = `https://${await assignFreeAlias(vercelDeploymentId, project)}`; } catch (error) { await recordEvent(deployment.id, deployment.projectId, "FREE_DOMAIN_WARNING", error instanceof Error ? error.message : String(error)); } }
...
export function freeDomainForProject(project: Project) { return `${normalizeFreeDomain(project.freeDomain, project.slug)}.vercel.app`; }
