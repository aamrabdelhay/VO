...
export async function setPlatformSecret(key: string, value: string, userId: string) {
  ...
  await db.execute(sql`
    insert into platform_secrets (id, key, cipher, last_four, updated_at, updated_by)
    values (${id}, ${key}, ${JSON.stringify(cipher)}::jsonb, ${trimmed.slice(-4)}, now(), ${userId})
  `);
  return id;
}
...
export async function getPlatformSecret(key: string): Promise<string | null> {
  assertAllowed(key);
  const values = await getPlatformSecretValues(key);
...
export async function getPlatformSecretById(id: string) {
  await ensureTable();
  const result = await db.execute<{ key: string; cipher: unknown }>(sql`select key, cipher from platform_secrets where id = ${id} limit 1`);
  const row = result.rows?.[0];
  if (!row?.cipher) return null;
  assertAllowed(row.key);
  try { return { key: row.key, value: decryptSecret(row.cipher) }; } catch { return null; }
}
...
export async function deletePlatformSecret(id: string) {
  await ensureTable();
...
async function checkResponse(key: string, response: Response, fallback: string): Promise<SecretCheck> {
  ...
  } catch { message = body.slice(0, 180) || fallback; }
...
export async function verifyPlatformSecret(key: string, valueOverride?: string): Promise<SecretCheck> {
  ...
  try {
    ...
    switch (key) {
      case "GITHUB_TOKEN": response = await fetch("https://api.github.com/user", { headers: { ...bearerHeaders(value), accept: "application/vnd.github+json" }, signal: controller.signal, cache: "no-store" }); return await checkResponse(key, response, "GitHub authentication failed");
      case "NVIDIA_API_KEY": response = await fetch("https://integrate.api.nvidia.com/v1/models", { headers: bearerHeaders(value), signal: controller.signal, cache: "no-store" }); return await checkResponse(key, response, "NVIDIA authentication failed");
      case "OPENROUTER_API_KEY": response = await fetch("https://openrouter.ai/api/v1/models", { headers: { ...bearerHeaders(value), "http-referer": "https://vo-lilac.vercel.app", "x-title": "VO Garvex" }, signal: controller.signal, cache: "no-store" }); return await checkResponse(key, response, "OpenRouter authentication failed");
      case "MISTRAL_API_KEY": response = await fetch("https://api.mistral.ai/v1/models", { headers: bearerHeaders(value), signal: controller.signal, cache: "no-store" }); return await checkResponse(key, response, "Mistral authentication failed");
      case "CEREBRAS_API_KEY": response = await fetch("https://api.cerebras.ai/v1/models", { headers: bearerHeaders(value), signal: controller.signal, cache: "no-store" }); return await checkResponse(key, response, "Cerebras authentication failed");
      case "GROQ_API_KEY": response = await fetch("https://api.groq.com/openai/v1/models", { headers: bearerHeaders(value), signal: controller.signal, cache: "no-store" }); return await checkResponse(key, response, "Groq authentication failed");
      case "KILO_API_KEY": response = await fetch("https://api.kilo.ai/api/gateway/chat/completions", { method: "POST", headers: { ...bearerHeaders(value), "content-type": "application/json" }, body: JSON.stringify({ model: "kilo-auto/free", messages: [{ role: "user", content: "ping" }], max_tokens: 1 }), signal: controller.signal, cache: "no-store" }); return await checkResponse(key, response, "Kilo authentication failed");
      case "COHERE_API_KEY": response = await fetch("https://api.cohere.com/v2/models", { headers: bearerHeaders(value), signal: controller.signal, cache: "no-store" }); return await checkResponse(key, response, "Cohere authentication failed");
      case "CLOUDFLARE_API_TOKEN": response = await fetch("https://api.cloudflare.com/client/v4/user/tokens/verify", { headers: bearerHeaders(value), signal: controller.signal, cache: "no-store" }); return await checkResponse(key, response, "Cloudflare token verification failed");
      case "VERCEL_DEPLOY_TOKEN": response = await fetch("https://api.vercel.com/v2/user", { headers: bearerHeaders(value), signal: controller.signal, cache: "no-store" }); return await checkResponse(key, response, "Vercel token authentication failed");
      ...
      case "OLLAMA_API_KEY": response = await fetch("https://ollama.com/api/tags", { headers: bearerHeaders(value), signal: controller.signal, cache: "no-store" }); return await checkResponse(key, response, "Ollama authentication failed");
